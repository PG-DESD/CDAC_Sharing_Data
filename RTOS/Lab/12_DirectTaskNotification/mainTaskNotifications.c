...
void sendingTask( void* NotUsed )
{
	while(1)
	{
		...
		From the eNotifyAction enumeration in task.h, we can see that the other options for
		notifying the task include the following:
		...
		/* Using these options creates some additional flexibility, such as using notifications as binary
		and counting semaphores. Note that some of these options change how xTaskNotify
		returns, so it will be necessary to check the return value in some cases.
		READ IT FIRST******************************READ IT FIRST******************************
