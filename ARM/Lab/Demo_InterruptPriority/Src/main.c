/**
  ******************************************************************************
  * @file    main.c
  * @author  Auto-generated by STM32CubeIDE
  * @version V1.0
  * @brief   Default main function.
  ******************************************************************************
*/

#if !defined(__SOFT_FP__) && defined(__ARM_FP)
  #warning "FPU is not initialized, but the project is compiling for an FPU. Please initialize the FPU before use."
#endif

#define IRQNO_TIM1_CC  27
#define IRQNO_TIMER4   30

#include<stdint.h>
#include<stdio.h>

/* NVIC register addresses. Refer to the processor generic guide */
uint32_t *pNVIC_IPRBase =  (uint32_t*)0xE000E400;
uint32_t *pNVIC_ISERBase = (uint32_t*)0xE000E100;
uint32_t *pNVIC_ISPRBase = (uint32_t*)0XE000E200;


void configure_priority_for_irqs(uint8_t irq_no, uint8_t priority_value)
{
	//1. find out iprx
	uint8_t iprx = irq_no / 4;
	uint32_t *ipr =  pNVIC_IPRBase+iprx;

	//2. position in iprx
	uint8_t pos = (irq_no % 4) * 8;

	//3. configure the priority
	*ipr &= ~(0xFF << pos);//clear
	*ipr |=  (priority_value << pos);

}

int main(void)
{
	//1. Lets configure the priority for the peripherals
	 configure_priority_for_irqs(IRQNO_TIM1_CC,0x80);
	 configure_priority_for_irqs(IRQNO_TIMER4,0x70);

	//2. Set the interrupt pending bit in the NVIC PR
	 *pNVIC_ISPRBase |= ( 1 << IRQNO_TIM1_CC);

	//3. Enable the IRQs in NVIC ISER
	 *pNVIC_ISERBase |= ( 1 << IRQNO_TIMER4);
	 *pNVIC_ISERBase |= ( 1 << IRQNO_TIM1_CC);


}

//isrs

void TIM1_CC_IRQHandler(void)
{
  printf("[TIM1_CC_IRQHandler Before Preemption]\n");
  /*Here, we are pending TIM4 interrupt request manually */
  *pNVIC_ISPRBase |= ( 1 << IRQNO_TIMER4);
  printf("[TIM1_CC_IRQHandler After Preemption]\n");
  while(1);

}


void TIM4_IRQHandler(void)
{
  printf("[TIM4_IRQHandler]\n");
}
